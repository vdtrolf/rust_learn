...
1. This is usefull after borrowing a a reference, and it is necessary to manipulate
...
2. For heap values, clone does the same job
> let mut heap_data = vec![5, 6, 7];  ^( mutable reference )^
> let ref2 = &mut heap_data;
> let deref2_copy = ref2.clone();
> ref2.push(8); ^( heap_data: [5, 6, 7, 8], deref2_copy: [5, 6, 7] )^
3. It is possible to combine dereferencing and borrowing:
> let mut x = 45;
> let z = &mut x;
> let y = &*z; ^( z = 45, y = 45 )^";
...
pub fn test_dereferencing() {
    ...
    *ref1 = 13; // mutates the original value
    println!("1. some_data: {some_data}, deref_copy: {deref_copy}");
    ...
    ref2.push(8);
    println!(
        "2. heap_data: {:?}, deref2_copy: {:?}",
        heap_data, deref2_copy
    );
